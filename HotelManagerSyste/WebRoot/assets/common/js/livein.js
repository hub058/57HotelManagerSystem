...
$(".modifyLivein").on("click",function(){
	var obj = $(this);
		var liveininno = obj.attr("liveininno");
		window.location.href="updateLivein.html?rinno="+ liveininno;		
});




$(".deleteLivein").on("click",function(){
	var obj = $(this);
	if(confirm("你确定要删除订单【"+obj.attr("liveininno")+"】及其所有的信息吗？")){
		$.ajax({
			type:"GET",
			url:"deleteLivein.html",
			data:{id:obj.attr("liveinid")},
			dataType:"json",
			success:function(data){
				if(data.delResult == "true"){//删除成功：移除删除行
					...
					alert("对不起，删除订单【"+obj.attr("liveininno")+"】失败");
				}else if(data.delResult == "notexist"){
					alert("对不起，订单【"+obj.attr("liveininno")+"】不存在");
				}
			},
			error:function(data){
				alert("对不起，删除失败");
