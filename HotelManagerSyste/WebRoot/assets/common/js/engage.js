...
$(".dssdfs").on("click",function(){
	var obj = $(this);
		var customercID = obj.attr("customercID");
		
		window.location.href="updateAcount.html?rchkID="+ customercID;		
});




$(".deleteEngage").on("click",function(){
	var obj = $(this);
	if(confirm("你确定要删除预定人【"+obj.attr("customercname")+"】及其所有的信息吗？")){
		$.ajax({
			type:"GET",
			url:"deleteEngage.html",
			data:{roomid:obj.attr("customercID")},
			dataType:"json",
			success:function(data){
				if(data.delResult == "true"){//删除成功：移除删除行
					...
					alert("对不起，删除订单【"+obj.attr("customercname")+"】失败");
