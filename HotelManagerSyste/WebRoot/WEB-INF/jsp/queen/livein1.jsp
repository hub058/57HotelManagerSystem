...
<td colspan="10" class="info_boxA"> <span style="color: red">部门级别:&nbsp;</span>${userSession.loginnum }　<span style="color: red">日期：</span><span   id=sTime></span>   
  <script>setInterval("document.all.sTime.innerText=new   Date().toLocaleString()",300)</script><a href="#" id="del_model" style="float: right;"><span>批量删除</span>  </td>
</tr>
<tr>
...
<th width="9%">操作</th>
   ...
   <c:forEach var="livein" items="${liveinList }" varStatus="status">
					...
					<td>
					<input type="checkbox" name="subChk" value="${livein.id}"/>
					</td>
						<td>
						${livein.id }
						</td>
...
<form method="post" action="${pageContext.request.contextPath }/livein1.html">
