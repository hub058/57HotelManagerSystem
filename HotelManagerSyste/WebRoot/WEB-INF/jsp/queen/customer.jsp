...
<script src="${pageContext.request.contextPath }/assets/common/js/customer.js" type="text/javascript"></script>
<style type="text/css">
...
<td colspan="10" class="info_boxA"> <span style="color: red">部门级别:&nbsp;</span>${userSession.loginnum }　<span style="color: red">日期：</span><span   id=sTime></span>   
  <script>setInterval("document.all.sTime.innerText=new   Date().toLocaleString()",300)</script> 　<div class="btn_box floatR mag_l20"><a href="${pageContext.request.contextPath }/user/addcustomer.html" style="color: #1FBBA6">添加会员</a></div><a href="#" id="del_model" style="float: right;"><span>批量删除</span>  </td></td>
</tr>
<tr>
...
<th width="7%">操作</th>
   ...
   <c:forEach var="customer" items="${custList }" varStatus="status">
					<tr>
					<td>
					<input type="checkbox" name="subChk" value="${customer.cID }"/>
					</td>
						<td>
						...
						<td>
						<span>${customer.csex}</span>
						</td>
						<td>
						<span>${customer.zjno}</span>
						</td>
						<td>
						<span>${customer.ctel}</span>
						</td>
						<td>
						<span>${customer.ctypename}</span>						
						</td>
						<td>
							<span>${customer.jifen}</span>
						</td>
						<td>
						...
						<td>
						<span><a class="modifyCustomer" href="javascript:;" customercID=${customer.cID }><img src="${pageContext.request.contextPath }/assets/statics/images/xiugai.png" alt="修改" title="修改"/></a></span>&nbsp;&nbsp;&nbsp;&nbsp;
						<span><a class="deleteCustomer" href="javascript:;" customercID=${customer.cID } customercname=${customer.cname }><img src="${pageContext.request.contextPath }/assets/statics/images/schu.png" alt="删除" title="删除"/></a></span>
						</td>
					</tr>
				</c:forEach>
...
<form method="post" action="${pageContext.request.contextPath }/user/getCustomer.html">
...
<span class="sttl">账号：</span>
<input name="querycID" class="input-text"  size="8"	type="text" value="${querycID }">
</div>
<div class="txtbox floatL mag_r20">
<span class="sttl">手机号：</span>
<input name="queryctel" class="input-text"  size="8"	type="text" value="${queryctel }">
</div>
</div>
<!-- btn_box -->
